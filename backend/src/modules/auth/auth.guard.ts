...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (isSkipAuth) {
      // Public route — still attach user when Bearer token is valid (e.g. canDelete on comments).
      if (token) {
        try {
          ...
          request['user']._meta = { accessToken: token };
        } catch {
          // Anonymous viewer — ignore invalid/expired token.
        }
      }
      return true;
    }
